...
- **Build Android natif** : **`apps/mobile/scripts/build-android-release.sh`** (assembleRelease, arm64, `-x lintVital*`, cache, JDK 17 pinné, `EXPO_PUBLIC_APP_ENV=production`, `open -R` à la fin ; `--clean` en option). À utiliser quand seul le JS/les assets changent → **pas de `expo prebuild`**. Dernier APK release (**2026-09-03 12:27**, signé clé debug, `ai.fixit.app` v0.1.0, ~52 Mo arm64, build **51 s**) = **étapes illustrées (schémas + repères sur photo) + refonte UI/UX du guide + i18n FR/EN du guide**, en plus de l'existant (i18n capture, lecture vidéo, pages légales OAuth, override guide admin, `RefineDiagnosis`).
