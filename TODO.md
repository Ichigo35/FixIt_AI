...
- ✅ **APK release PHASES 11+12+13** reconstruit + **allégé 52 Mo → 23 Mo** (2026-09-07). Leviers dans `build-android-release.sh` (réinjectés après `expo prebuild`) : `.so` compressés (`expo.useLegacyPackaging=true`, −17 Mo), R8 minify + `shrinkResources` (dex 56→23 Mo) + garde-fous proguard, natif ML Kit code-barres retiré au packaging (`libbarhopper_v3.so` −4,9 Mo), `expo.gif.enabled=false`. Bundle vérifié frais + classes natives critiques présentes après R8 (`AudioModule`, `CameraViewModule`, `VideoModule`, `SecureStoreModule`, `ImageManipulatorModule`, Hermes, Reanimated). **R8 à valider sur device** (démarrage app + 1 diagnostic complet). Reste ~0,6 Mo de modèles tflite ML Kit (assets, exclusion sans effet — marginal).
