...
- **Build Android natif** : `expo prebuild` + `./gradlew :app:assembleRelease` en local. Dernier APK release (2026-09-02 20:03, signé clé debug, `ai.fixit.app` v0.1.0, ~52 Mo arm64) inclut i18n flux de capture + lecture vidéo `expo-video` + pages légales OAuth + **override guide admin + `RefineDiagnosis`**. Build incrémental (sans `--clean`, daemon Gradle froid) = **~14 min** ce jour-là. Voir « Build mobile » ci-dessous.
...
**À faire :** redéployer le Worker pour propager la var `GEMINI_FALLBACK_MODEL` en prod (`pnpm --filter @fixit/api run deploy`, à lancer par l'utilisateur) · rebuild APK release pour voir le guide illustré sur device (JS seulement, pas de build natif) · tester vidéo end-to-end sur device (File API réelle + lecture `expo-video`) · système d'abonnement `plan` FREE/PREMIUM pour les non-admins · EAS Build · i18n sur les écrans restants (`DiagnosisResultView`/`StopView`/`OutcomeSession`/`RepairGuideView` + `lib/errors.ts`) · domaine custom = abandonné.

**Prod :** Worker version `7444aa66` (2026-09-02) — `ADMIN_EMAILS` + pipeline vidéo (PHASE 9) + `/auth/callback` + pages légales publiques `/` `/privacy` `/terms` + **override guide admin** (`repair-guide`/`repair-session`/`verify` sans 409 `forced_stop` pour un admin). `/health` OK (`env=production`). **Pas encore déployé** : var `GEMINI_FALLBACK_MODEL` (bascule modèle Gemini) — attend le prochain `deploy`.
