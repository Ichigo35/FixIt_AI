...
- **110 tests** (44 shared + 38 api + 28 mobile). Base Neon : 1 user (`tcha.jimmy@gmail.com`, admin). Dép. mobile ajoutée : `expo-video` (~57.0.3, plugin dans `app.config.ts`).
...
**Prod à jour :** Worker version `7444aa66` (2026-09-02) — `ADMIN_EMAILS` + pipeline vidéo (PHASE 9) + `/auth/callback` + pages légales publiques `/` `/privacy` `/terms` + **override guide admin** (`repair-guide`/`repair-session`/`verify` sans 409 `forced_stop` pour un admin). `/health` OK (`env=production`).
