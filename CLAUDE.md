...
- **Build Android natif** : `expo prebuild` + `./gradlew :app:assembleRelease` en local. Dernier APK release (**2026-09-02 23:31**, signé clé debug, `ai.fixit.app` v0.1.0, ~52 Mo arm64) ajoute le **guide de réparation illustré** (icônes outils + type d'étape) — sur celui de 20:03 (i18n capture + lecture vidéo + pages légales OAuth + override guide admin + `RefineDiagnosis`). Rebuild **sans `expo prebuild`** (changement JS/assets seulement, aucun module natif) : juste `./gradlew :app:assembleRelease` dans `apps/mobile/android/`. Build incrémental daemon Gradle froid ce jour-là = **~19 min** (`compileReleaseKotlin` + `lintVitalRelease` se sont quand même relancés). Voir « Build mobile » ci-dessous.
